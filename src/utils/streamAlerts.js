  ...
  for (const [guildId, guild] of client.guilds.cache) {
    ...
    for (let i = 0; i < streamers.length; i++) {
      try {
        ...
        // Eliminar datos de transmisión si el streamer ya no está en vivo
        if (!liveInfo.isLive && lastLiveData.has(liveStreamKey)) {
          lastLiveData.delete(liveStreamKey);
        }

        // Actualizar la información del streamer en la configuración del gremio
        streamers[i] = {
          ...streamers[i],
          ...liveInfo.streamer,
          lastLiveAt: liveInfo.isLive ? new Date() : streamers[i].lastLiveAt,
        ...
        console.error(`Error during live check for ${streamers[i]?.name || streamers[i]?.id}:`, error);
      }
    }
  }
}
...
function createStreamerEmbed(streamer) {
  ...
  let description = `@everyone ${streamer.username || streamer.name} está en directo por [${streamer.platform}](${streamer.url}).`;
  ...
  if (streamer.startedAt) {
    ...
    fields.push({
      name: "⏱️ Comenzó hace",
      value: `${Math.floor((Date.now() - startedAtDate.getTime()) / 60000)} minutos`,
      inline: true,
    });
  }
